...
public class ViewCommandBomberman extends JPanel implements ActionListener, Observer {
	...
	public ViewCommandBomberman(BombermanGame game, ControleurBombermanGame csg) {
		...
		JPanel panGenGen = new JPanel();
		panGenGen.setLayout(new GridLayout(2,1));
		
		JPanel panInfos= new JPanel();
		panInfos.setLayout(new GridLayout(1,3));
		...
		//Labels d'infos du joueur
		JLabel jl_utilisateur = new JLabel(_game.getPseudo(),JLabel.CENTER);
		JLabel jl_niveau = new JLabel(_game.getNiveau(),JLabel.CENTER);
		JLabel jl_scoreMax = new JLabel(_game.getScoreMax(),JLabel.CENTER);
		JLabel jl_cheminAvatar = new JLabel(_game.getCheminAvatar(),JLabel.CENTER);
		...
		panGen.add(panBas);
		//jFrame.add(panGen);
		//Ajout des infos du joueru dans le panel Infos
		panInfos.add(jl_utilisateur);
		panInfos.add(jl_niveau);
		panInfos.add(jl_scoreMax);
		panInfos.add(jl_cheminAvatar);
		panGenGen.add(panGen);
		panGenGen.add(panInfos);
